...
const Samp = ({ userData, togs, setTogs }) => {

    const [hide,setHide] = useState(false)
    const [checkNav, setCheckNav] = useState(1)
    return (
        <>
            <div className='relative max-w-[90rem]  border-[1px border-green-500 h-16 mx-auto min-w-sm border-[1px z-40'>

                <div className='absolute tracking-wide inset-y-0 left-7 md:left-10 lg:left-14 xl:left-20  border-[1px flex gap-5 items-center justify-between h-16'>
                ...
                <div className='absolute inset-y-0 right-7 md:right-10 lg:right-14 xl:right-20  flex gap-5 items-center justify-between h-16'>
                    <h1 className={`border-[1px px-4 pointer text-[#c7c5c5] hover:text-zinc-100 uppercase font-semibold tracking-wider font-pops 
                                ...
                                ${!hide && 'after:bg-zinc-500 after:w-[100%]'}
                                `}>
                    ...
                    <div className={`absolute flex flex-col bg-zinc-600/30 rounded-xl gap-3 border-[1px -left-0 top-20 w-32 px-3 py-5 
                                     ${hide && 'hidden'}  `}>
